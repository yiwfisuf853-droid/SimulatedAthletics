...
| `src/components/layout/LeftPanel.tsx` | 删除 | 4 |
...
**当前**：旧版 `LeftPanel` 已移除；新版布局不再以常驻左侧栏承载设置。

**目标**（新版 UI 规范）：

| phase | 左侧 | 中间 | 右侧 |
|-------|------|------|------|
| idle | 无独立侧栏 | ConfigBar + IntroMessage / MoreSettingsPanel | Timer + Score + Leaderboard |
| playing | 无独立侧栏（键盘谱贴近棋盘显示） | GameGrid + KeyDisplay + KeyboardScorePanel | Timer + Score + Leaderboard(禁点击) |
| finished/final_clicked | 无独立侧栏（键盘谱继续保留） | GameGrid + 覆盖层 + KeyboardScorePanel | Timer + Score + Leaderboard(可点击) |
| replay | 无独立侧栏（复盘辅助贴近棋盘） | ReplayGrid + KeyDisplay + KeyboardScorePanel | Timer + Score + ReplayPanel |

调整方向：
```
idle → 设置能力全部进入顶部配置区或 MoreSettingsPanel
replay → 复盘控制进右侧，键盘谱贴近棋盘并完整展示
playing/finished/final_clicked → 键盘谱保留完整序列，单键模式只强调主键
```

### 4.2 删除 `src/components/layout/LeftPanel.tsx`

处理原则：
- 旧侧栏代码已删除，避免新版布局继续受历史结构牵制
- 空闲态设置应逐步迁移到顶部配置区与 MoreSettingsPanel
- 对局 / 复盘辅助信息应贴近棋盘区，而不是固定成左侧工具栏
